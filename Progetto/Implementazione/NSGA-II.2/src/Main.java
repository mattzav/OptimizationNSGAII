...
public class Main {
	...
	private static int populationSize = 500 * 2; // numero pari
	private static double mutationProb = 0.2;
	private static double crossoverProb = 0.9;
	private static int numIteration = 200;
	...
	private static double maxBatteryConsumption = 200;

	public static Graph graph;
	public static int numNodesInTheGraph = 200;
	private static int numExtraNodesForDroneInTheGraph = 100;
	private static int numMaxVehicles = 30;
...
//			System.out.println(busiest.getCurrentDroneTourLength());
			...
			for (int index = 0; index < s.getGenotypeVehicles().size(); index++) {
				Vehicles v = s.getGenotypeVehicles().get(index);
				if (v.getCurrentCapacity() >= graph.getNeededResource(toAssign) && v.getTourLength()
						+ graph.getNormalizedDuration(v.getTour().get(v.getTour().size() - 1), toAssign)
					...
					System.out.println("duration "+graph.getNormalizedDuration(v.getTour().get(v.getTour().size() - 1), toAssign));
					System.out.println("drone tour "+droneTour);
	...
	private static void mutation(Individual individual) {
		...
		for (int i = numNodesInTheGraph; i < numNodesInTheGraph + numExtraNodesForDroneInTheGraph; i++) {
			...
			if (extractedRandomVehicle.getCurrentDroneTourLength().get(extractedRandomNodeForAdding)
					+ graph.getNormalizedDuration(extractedRandomNodeForAdding,
				...
				extractedRandomVehicle.addExtraNode(extractedRandomNodeForAdding, extractedRandomNode,
						graph.getNormalizedDuration(extractedRandomNodeForAdding, extractedRandomNode),
		...
		for (int i = numNodesInTheGraph; i < numNodesInTheGraph + numExtraNodesForDroneInTheGraph; i++) {
			...
			if (currentProfit - graph.getProfit(extractedDroneNode) >= minProfitNeeded) {
				extractedRandomVehicle.removeExtraNode(extractedRandomNode, extractedDroneNode,
						graph.getNormalizedDuration(extractedRandomNode, extractedDroneNode),
	...
	private static ArrayList<Individual> initPopulation() {
		...
		for (int i = 0; i < populationSize; i++) {
			...
			if (i == 0) {
				...
				copyNodesDrones = new ArrayList<Integer>(nodesForTheDrones);
			}
