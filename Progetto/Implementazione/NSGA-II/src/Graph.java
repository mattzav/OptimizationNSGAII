...
public class Graph {
	...
	public Graph(int numNodes) {
		...
		neededResource = new int[numNodes];
		profit = new double[numNodes];
		arcs = new DurationParameters[numNodes][numNodes];
		initRandom();
	}
	...
	private void initRandom() {
		for (int i = 0; i < numNodes; i++) {
			...
			for (int j = 0; j < numNodes; j++)
				arcs[i][j] = new DurationParameters(((Main.r.nextInt() % 20) + 20) % 20,
	...
	public double getNormalizedDuration(int i, int j) {
